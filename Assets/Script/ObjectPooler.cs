...
public class ObjectPooler : MonoBehaviour
{
    ...
    GameObject _SpawnFromPool(string tag, Vector3 position, Quaternion rotation)
    {
        ...
        if (poolQueue.Count <= 0)
        {
            ...
            var obj = CreateNewObject(pool.tag);
            ArrangePool(obj);
        }
        ...
        {
            rectTransform.localPosition = position; // RectTransform의 위치 설정
            rectTransform.rotation = rotation; // RectTransform의 회전 설정
        }
        else
        {
            objectToSpawn.transform.position = position; // 일반 Transform의 위치 설정
            objectToSpawn.transform.rotation = rotation; // 일반 Transform의 회전 설정
        }
    ...
    {
        GameObject loadedPrefab = Resources.Load<GameObject>($"Prefabs/{tag}");
        if (loadedPrefab == null)
        {
            Debug.LogError($"프리팹을 찾을 수 없습니다. 경로를 확인하세요: Prefab/{tag}");
            return null;
        }
